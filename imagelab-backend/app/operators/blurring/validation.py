...
def validate_positive_odd_kernel_size(value: int, name: str) -> None:
    ...
    if value <= 0:
        raise ValueError(f"'{name}' must be a positive odd integer, got {value}. Use a value like 1, 3, 5, 7, …")
    if value % 2 == 0:
        raise ValueError(
            f"'{name}' must be a positive odd integer, got {value} (even). Did you mean {value - 1} or {value + 1}?"
        )
...
def validate_median_kernel_size(value: int, name: str) -> None:
    ...
    if value <= 1:
        raise ValueError(f"'{name}' must be an odd integer greater than 1, got {value}. Use a value like 3, 5, 7, …")
    if value % 2 == 0:
        raise ValueError(
