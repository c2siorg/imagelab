...
def test_face_and_smile_boxes_have_distinct_colors():
    """Face boxes and smile boxes should have distinct colors when both are drawn."""
    op = make_operator({"drawFaceBoxes": True, "rgbcolors_input": "#00FF00"})
    image = make_blank_image(channels=3, height=200, width=200)

    # Mock face_cascade and smile_cascade to simulate detections
    op.face_cascade = MagicMock()
    op.face_cascade.detectMultiScale.return_value = np.array([[20, 20, 80, 80]])
    op.smile_cascade = MagicMock()
    op.smile_cascade.detectMultiScale.return_value = np.array([[10, 10, 30, 20]])
    ...
    # Smile color is green (#00FF00 -> BGR (0, 255, 0))
    # Face color is distinct blue BGR (255, 100, 0)
    has_green = np.any((result[:, :, 0] == 0) & (result[:, :, 1] == 255) & (result[:, :, 2] == 0))
    has_blue = np.any((result[:, :, 0] == 255) & (result[:, :, 1] == 100) & (result[:, :, 2] == 0))
    assert has_green, "Smile bounding box with user color was not drawn"
    assert has_blue, "Face bounding box with distinct color was not drawn"


def test_float_image_with_detections_returns_uint8():
    """Float image with detected face/smile should return uint8 result."""
    op = make_operator({"drawFaceBoxes": True})
    image = np.ones((200, 200, 3), dtype=np.float32) * 0.5
